...
# Set buffer size to the total number of English sentences
BUFFER_SIZE = len(english_sentences)

# Define batch size for training
BATCH_SIZE = 64

# Create a random train-test split (80% train, 20% validation)
is_train = np.random.uniform(size=(len(portuguese_sentences),)) < 0.8

# Prepare training dataset: shuffle and batch sentence pairs for training
train_raw = (
    tf.data.Dataset.from_tensor_slices(
        (english_sentences[is_train], portuguese_sentences[is_train])
    )
    .shuffle(BUFFER_SIZE)  # Shuffle the entire dataset
    .batch(BATCH_SIZE)     # Batch sentences in groups of 64
)

# Prepare validation dataset: shuffle and batch sentence pairs for validation
val_raw = (
    tf.data.Dataset.from_tensor_slices(
        (english_sentences[~is_train], portuguese_sentences[~is_train])
    )
